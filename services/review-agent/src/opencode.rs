...
pub enum ReviewResolution {
    ...
    /// The model called `abort` — incomplete and unverified; the host clears findings and posts the reason.
    Aborted(String),
}
...
/// Pure drive-loop control for the OpenCode review host: given each observed cycle outcome, decide
/// whether to re-prompt (bounce or keep-going) or finalize. Wraps [`ReviewGates`] with the
/// supervisor's re-prompt budget so the async transport shell (spawn / `session/prompt` / recorder
/// tail) stays a thin I/O layer over this testable core.
...
mod tests {
    ...
    fn finish_cycle(summary: &str) -> TurnOutcome {
        cycle_turn_outcome(&[
            before("lightbridge_finish", "f", serde_json::json!({ "summary": summary })),
    ...
    fn driver_bounces_then_finalizes_finished() {
        ...
        match driver.on_cycle(&finish_cycle("lgtm")) {
            DriveAction::Prompt(nudge) => assert!(nudge.contains("a.rs")),
            other => panic!("expected a bounce Prompt, got {other:?}"),
        }
        let covered = cycle_turn_outcome(&[
            before("lightbridge_read_file", "r", serde_json::json!({"path": "a.rs"})),
            after("lightbridge_read_file", "r", "source"),
            before("lightbridge_finish", "f2", serde_json::json!({"summary": "done"})),
    ...
    fn driver_finalizes_aborted_immediately() {
        ...
        let abort = cycle_turn_outcome(&[
            before("lightbridge_abort", "a", serde_json::json!({"reason": "no PR diff"})),
