...
use lci_agent_types::{ToolCallReq as ToolCall, ToolSpec as ToolDef};
...
/// Build the typed registry used by the extracted loop in R1d. The current Job loop continues
/// through [`Tools`] below, and both paths share that exact dispatcher implementation.
pub fn tool_registry(
    client: Arc<ControlPlaneClient>,
    embedder: Arc<EmbeddingsClient>,
    discovered: impl IntoIterator<Item = ToolDef>,
    caps: RuntimeCaps,
) -> Result<ToolRegistry, RegistryError> {
    let mut registry = ToolRegistry::new();
    for spec in tool_defs().into_iter().chain(discovered) {
        let (kind, replay) = tool_contract(spec.name());
        registry.register(
            Arc::new(ReviewTool {
                spec,
                kind,
...
fn tool_contract(name: &str) -> (ToolKind, ReplaySafety) {
    match name {
        VECTOR_SEMANTIC_SEARCH | GRAPH_FIND_SYMBOL | GRAPH_GET_CALLERS => (
            ToolKind::ReadOnly(ReadKind::Retrieval),
            ReplaySafety::ReadOnly,
        ),
        READ_FILE => (ToolKind::ReadOnly(ReadKind::File), ReplaySafety::ReadOnly),
        ADD_REVIEW_COMMENT | RETRACT_FINDING => (ToolKind::Write, ReplaySafety::Idempotent),
        ADD_COMMENT => (ToolKind::Write, ReplaySafety::NeedsDedupKey),
        FINISH | ABORT => (ToolKind::Terminal, ReplaySafety::Idempotent),
        REPORT_PROGRESS => (ToolKind::Progress, ReplaySafety::Idempotent),
...
impl Tool for ReviewTool {
    ...
    fn call<'a>(&'a self, cx: &'a ToolCx<'a>, args: &'a str) -> BoxFuture<'a, ToolOutcome> {
        Box::pin(async move {
            let root = if self.spec.name() == READ_FILE {
                match cx.workspace.root().await {
                    Ok(root) => root,
                    Err(error) => {
            ...
            legacy
                .dispatch(&ToolCall {
                    id: cx.dedup_key.unwrap_or("registry-call").to_string(),
...
mod tests {
    use super::*;
    use lci_agent_tools::{TurnFilter, Workspace, WorkspaceError};
    use lci_agent_types::FunctionCallReq as FunctionCall;
    ...
    #[test]
    fn legacy_goldens_expand_to_the_exact_current_tool_specs() {
        let catalog = tool_defs();
        for scenario in lci_agent_testkit::GoldenScenario::ALL {
            let baseline = lci_agent_testkit::GoldenHarness::legacy_baseline(scenario, &catalog);
            for (names, specs) in baseline
                .offered_tool_names
                .iter()
                .zip(&baseline.offered_specs)
            {
                assert_eq!(names.len(), specs.len());
                for (name, spec) in names.iter().zip(specs) {
                    assert_eq!(spec["function"]["name"], name.as_str());
    ...
    impl Workspace for TestWorkspace {
        fn root(&self) -> BoxFuture<'_, Result<&Path, WorkspaceError>> {
            Box::pin(async move {
                if self.fail {
                    Err(WorkspaceError::new("checkout unavailable"))
                } else {
    ...
    #[tokio::test]
    async fn typed_registry_preserves_contracts_and_delegates_to_legacy_dispatch() {
        assert_eq!(
            known_tool_names(),
            tool_defs().iter().map(|s| s.name()).collect::<Vec<_>>()
        );
        let discovered = ToolDef::function(
            "mcp__docs__lookup",
            "external docs",
            serde_json::json!({"type":"object"}),
        );
        let cp = Arc::new(ControlPlaneClient::new("http://unused", "tok"));
        let emb = Arc::new(EmbeddingsClient::new("http://unused", "key", "model"));
        ...
        assert!(matches!(
            view.dispatch(&cx, &call(ABORT, r#"{"reason":"done"}"#)).await,
            ToolOutcome::Abort(reason) if reason == "done"
        ));
        assert!(matches!(
            view.dispatch(&cx, &call(READ_FILE, r#"{"path":"a.rs","start_line":2}"#)).await,
            ToolOutcome::Continue(text) if text.contains("two")
        ));

        let failed_workspace = TestWorkspace {
            root: dir.path().to_path_buf(),
            fail: true,
        };
        let failed_cx = ToolCx {
            task_id: Uuid::nil(),
            workspace: &failed_workspace,
            dedup_key: None,
        };
        assert!(matches!(
            view.dispatch(&failed_cx, &call(READ_FILE, r#"{"path":"a.rs"}"#)).await,
            ToolOutcome::Continue(text) if text.contains("could not materialize")
        ));
        ...
        assert!(matches!(
            ...
            Err(RegistryError::MissingDedupCapability(name)) if name == ADD_COMMENT
        ));
        assert_eq!(
            tool_contract("future_read"),
            (
                ToolKind::ReadOnly(ReadKind::Knowledge),
                ReplaySafety::ReadOnly
            )
