...
| R2 | PDF parsing on untrusted repo input crashes/OOMs the index Job or is exploited via a crafted PDF | Medium | Medium | Bound time + memory + output size per file; catch/skip on parse failure and log; runs in the index Job, which is the sandboxed one; pick a maintained crate (Unresolved) and treat it as untrusted-input code |
