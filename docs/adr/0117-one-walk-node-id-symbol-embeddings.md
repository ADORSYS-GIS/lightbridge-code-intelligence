# ADR-0117: One walk, and a symbol's vector rides the chunk that is its body
...
- **Neutral** — chunk-shape tuning moves from `INDEX_MAX_CHUNK_LINES` / `INDEX_WINDOW_SIZE` /
  `INDEX_WINDOW_STEP` / `INDEX_MAX_CHUNK_BYTES` to `lci-codegraph`'s own `LCI_CODEGRAPH_*`
  equivalents. None of the four is set in any deployed environment today; `INDEX_EMBED_BATCH_SIZE`,
  which is, keeps its name and meaning.
- **Neutral** — `GraphNodeInput.embedding` stays on the control-plane side. A current runner never
  sends it, but keeping it means a mixed-version rollout degrades rather than breaks, and `None`
  already means "leave any existing vector alone".
- **Neutral** — 1.7% of symbol nodes still get no vector (a graph node with no corresponding chunk).
  Accepted, against 38% previously carrying a wrong one.
