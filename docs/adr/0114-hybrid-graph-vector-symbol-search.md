...
- **Status:** Accepted — index-time symbol embedding amended by
  [ADR-0116](0116-one-walk-node-id-symbol-embeddings.md) (one walk; the vector rides the chunk
...
[ADR-0116](0116-one-walk-node-id-symbol-embeddings.md) — the vector now arrives on the chunk that is
...
- [ADR-0089](0089-embeddings-on-the-code-graph.md) — the original symbol-embeddings proposal this ADR
  makes concrete.
- [ADR-0116](0116-one-walk-node-id-symbol-embeddings.md) — retires this ADR's "`lci-codegraph` stays
