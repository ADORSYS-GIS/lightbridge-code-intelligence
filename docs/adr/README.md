...
| [0082](0082-restate-durable-agent-runtime.md) | Restate Phase D (RFC-0005, new phase — the stated end goal of adopting the engine): the deep-tier review loop becomes a durable `ReviewAgent` workflow — every LLM call + tool batch a journaled `ctx.run` step, so a crash resumes at the step it left (O(1-turn) cost, not O(run)); hosted by a new `agent-worker` role (fast/index keep the ADR-0004 Job; SAST stays sandboxed in a Job); journal-offload rule, `ensure_checkout` lazy local-state guard, per-tool idempotency table; gated G0–G4 (Phase B soaked, journal-scale + cross-pod-resume spikes, revision-drain rehearsal, `add_comment` dedup key); fallback = Option B transcript-checkpoint rehydration | Proposed |
