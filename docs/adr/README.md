...
| [0083](0083-platform-crate-architecture-and-cratestack-data-layer.md) | Platform crate architecture (extends ADR-0082 to every component; amends ADR-0005): the ~25.5k-line control-plane decomposes into domain crates `lightbridge-{data,platform,egress,queue,a2a}` + thin host (deps pinned to single manifests: cratestack-pg/sqlx→data, kube→queue); **NEW `shared` role** = all six roles composed in one process for single-node/customer installs, with **Postgres advisory locks making every singleton (egress drain, notifier claim, dispatcher ticks) structural** instead of replicas:1 comments; **cratestack 0.4.9 adopted as the high-level ORM** (ORM-only: `include_server_schema!` delegates behind per-domain repository traits, generated routes NOT mounted, raw-SQLx escape hatch for SKIP-LOCKED/NOTIFY/CTE/pgvector residue, `.cstack` regenerated from the live 27-migration schema + CI drift gate, existing migration pipeline kept); P0–Pn sequencing with P1 = cratestack parity spike (fallback: repos stay raw SQLx); risks R20–R24 | Proposed |
