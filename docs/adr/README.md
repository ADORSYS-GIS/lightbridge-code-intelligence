...
| [0071](0071-platform-abstraction-layer.md) | Platform-abstraction layer (`CodePlatform` trait) for GitHub + GitLab — one async trait, two implementations, unified `/webhook` route + `outbox` table with a `platform` column; agent-runner untouched; phases 0–3 are pure refactors (zero GitHub behavior change) | Accepted |
