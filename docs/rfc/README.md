...
| [0008](0008-gitlab-multi-forge.md) | GitLab / multi-forge — **retrospective**: the `Forge` boundary #253 asked for already shipped as [ADR-0072](../adr/0072-platform-abstraction-layer.md) (`CodePlatform` trait, `GithubApp`/`GitlabClient`, unified webhook+outbox); maps GitHub vs GitLab across ingress/forge-auth/egress/identity, corrects a ticket conflation between forge-auth and ADR-0092's unrelated internal runner token, and gives a build/no-build call on the three real remaining gaps — GitLab's single static-token blast radius (no-build, operational mitigation only), single-tenant GitLab config (**already in flight as PR #414**, deferred to it), `open`-mode write-back not yet in `CodePlatform` (deferred to ADR-0088) | Proposed |
| [0009](0009-opencode-acp-agent-host.md) | OpenCode-over-ACP as the agent-loop host — the third (and this time evidence-gated) OpenCode round: OpenCode owns the LLM loop/provider quirks/MCP-client/subagents, we keep the trust boundary (mediated MCP tools, unchanged), the transcript (recorder plugin) and the gates (interlock plugin, block-until on `tool.execute.before` — enforced, not honor-system); **step replay deliberately dropped** for hosted modes (ADR-0087's `CheckpointRuntime` never exercised in prod — the ADR-0093 judge-by-exercised-rationale lesson applied to our own runtime); phased: probe (checklist a–f, gate) → `open` mode ([ADR-0094](../adr/0094-opencode-acp-open-mode-host.md)) → review eval harness → review/index hard cutover (own ADR); plugins under `integrations/opencode/plugins/*` ([ADR-0095](../adr/0095-opencode-plugins-recording-and-gates.md)) | Draft |
