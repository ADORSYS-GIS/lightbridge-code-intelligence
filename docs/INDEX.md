...
- [Review pipeline](review-pipeline.md) — the whole review subsystem end to end: preset selection (repo-configurable fast/deep/ultra, [ADR-0103](adr/0103-repo-configurable-opencode-review-presets.md), superseding the old fixed two-tier model, [ADR-0062](adr/0062-two-tier-review-fast-auto-deep-on-demand.md)), the opengrep SAST tool (`run_sast`, [ADR-0073](adr/0073-sast-as-agent-tool.md)), the OpenCode-hosted mediated-tools agent loop (live since 2026-07-17, [ADR-0097](adr/0097-review-runs-on-opencode.md); the native loop is the fallback), control-plane finalize/shaping, and egress. (Prose in `review-pipeline.md` itself still describes the pre-ADR-0103 two-tier model pending Epic #491's implementation — treat the ADR as authoritative until that doc is rewritten.)
