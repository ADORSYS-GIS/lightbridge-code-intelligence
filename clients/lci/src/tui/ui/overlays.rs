...
pub(super) fn draw_help(f: &mut Frame, theme: &Theme) {
    ...
    let lines = vec![
        ...
        row("r", "refresh the task + review"),
        section("Actions"),
        row("a / d / c", "approve · deny (purges index) · cancel run"),
        row("s", "open the selected repo's review preset settings"),
        section("Repo settings"),
        row("type / ⌫", "edit the new preset name"),
        row("↵", "commit it to .lightbridge-code-review.jsonc"),
        row("Esc / q", "back to the Repositories list"),
