...
/**
 * How a repository is reviewed: its preset, the individual settings each independently resolved
 * across a built-in default → repo config file → admin override chain, and which model the preset
 * runs on.
 *
 * Everything here writes. Grouping the writes together is what lets the one thing a reader without
 * permission needs to know — that they cannot change any of it — be said once, at the top, instead
 * of repeated beside each control it disables.
 */
export default async function RepositorySettings({ params }: { params: Promise<{ id: string }> }) {
  const { id: rawId } = await params;
  const id = Number(rawId);
  if (!Number.isInteger(id) || id <= 0) notFound();

  const [presetResult, settingsResult, modelResult, allowlistResult, claims] = await Promise.all([
    getRepoPreset(id),
    getRepoSettings(id),
    getRepoModel(id),
    getModelAllowlist(),
  ...
  return (
    <>
      {/* Said once for the whole view. The control plane is the real gate; disabling the controls
          only avoids offering an action that could not succeed. */}
      {!canConfigure && (
        <Card>
          <StatusLine>
            You need the <code>repo:configure</code> permission to change this repository's review
            configuration. Ask an administrator to grant it.
          </StatusLine>
        </Card>
      )}
      ...
      {/* Dropped entirely rather than shown disabled: a card headed "Change preset" that offers no
          way to change one is a worse answer than not raising the possibility, and the notice above
          has already given the reason. */}
      {canConfigure && (
        <Card>
          <CardHeader>
        ...
        <CardBody>
          {!modelResult.ok ? (
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {/* Model selection is operator-cost-relevant rather than a repo-owner setting, so it
                  carries its own permission and its own note — the page-level one above does not
                  cover it. */}
              {!canConfigureModel ? (
                <StatusLine>
